...
**作成日**: 2026年2月12日
**最終更新**: 2026年2月21日（数学的根拠を実証、法線一致時のbz=0を検証）
...
### 3. ✅ 数学的解明（2026年2月21日更新）

#### 因果関係の証明

**従来**: 「凸円弧でdz=0が経験的に機能する」という観察結果のみ

**2026年2月21日**: **数学的根拠を実証**（詳細は `docs/bisector_algorithm_mathematical_analysis.md` 参照）
...
ここで `b̂z` はbisectorのZ成分。

**数値検証**（`bisector_bz_verification.test.ts`）:
```
ノード2（arc凸→line）での実測:
  bz = -0.7068
  dist = 0.4mm（ノーズR）
...
**法線が異なる接続**（例: 直線→直線、円弧→テーパー）:
```
b̂z ≠ 0
→ Pz = refZ + b̂z × dist ≠ refZ
→ Z方向オフセットが必要（dz=noseR）
```

#### ⚠️ 重要な限定条件

**✅ 実証されたこと**:
- 数式 `Pz = refZ + bz × dist` の正しさ
- 法線一致時に `bz = 0` となること
- 数値的に完全一致（-0.2827mm）

**❌ 一般化の誤り**:
- 「凸円弧のノードでは常に bz=0」 ← **誤り**
- 正しくは「**法線が一致する接続**（例: 垂直線→凸円弧）で bz=0」

**検証された条件**:
- 外径加工、Tip 3のみ
- 垂直線→凸円弧の特定ケース
- 両セグメントの法線がZ成分ゼロ（水平方向）
