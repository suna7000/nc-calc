...
$P = O + V_{offset}$ （半径値）の定義は以下の通りです。

| チップ番号 | Xオフセット | Zオフセット（※） | 物理定義 (Rear基準) |
|:---:|:---:|:---:|:---|
| **3** | $+R$ | $+R$ (凹/直線)<br>$0$ (凸) | 外径/前/右勝手（標準） |
| **2** | $-R$ | $-R$ (凹/直線)<br>$0$ (凸) | 内径/前/右勝手（標準） |
| **1** | $-R$ | $+R$ (凹/直線)<br>$0$ (凸) | 内径/奥 |
| **4** | $+R$ | $-R$ (凹/直線)<br>$0$ (凸) | 外径/奥 |
| **8** | $+R$ | $0$ | 端面/X方向逃げ |

**※ 重要**: Zオフセットは二等分線法使用時、コーナータイプ（凸/凹）により異なる（詳細は上記セクション参照）
...
### 🔴 重要：凸/凹コーナーでのZ方向オフセット適用の違い（2026年2月修正）

**問題**: 二等分線法で計算した工具中心座標(P)からプログラム座標(O)への変換時、一律にZ方向オフセット(`dz = noseR`)を適用すると、凸コーナー(角R)で系統的な誤差が発生する。

**原因**: 二等分線法は凸コーナーの幾何学的オフセットを**既に正しく計算済み**であり、追加のZ方向オフセットは不要。一方、凹コーナー(隅R)や直線では追加オフセットが必要。

**修正後の実装ルール（pToO関数）**:

```javascript
// Tip 3 (外径/前向き) の場合
function pToO(px, pz, noseR, toolType, isConvex) {
  const dx = noseR  // X方向は常に適用
  const dz = isConvex ? 0 : noseR  // ← ここが重要！

  return {
    ox: px - dx * 2,  // 直径値換算
    oz: pz - dz       // 凸コーナーでは0、凹/直線ではnoseR
  }
}
...
**検証結果（角R0.5、noseR=0.4mm）**:
- 修正前: Z方向誤差 **-0.366mm** (≈ -noseR)
- 修正後: Z方向誤差 **+0.034mm** (91%改善)
